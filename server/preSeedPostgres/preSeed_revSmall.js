...
var dirPath = path.join(__dirname, `/data/reviews${fileCount}.txt`);
...
for(let dishID = 1; dishID <= DISH_MAX; dishID++){ 
    ...
    for(let reviewNum = 0; reviewNum < REVIEW_MAX; reviewNum++){
        ...
        fs.appendFileSync(dirPath, reviewConcat, function(err) {
            ...
            let dishIDmod = dishID % 2000;

            if(!(dishIDmod === 0 && reviewNum === REVIEW_MAX-1)){
    ...
    if(dishID % 2000 === 0 ) {
        // we reached 1 mil
        console.log("write NEW FILE: ", dishID*REVIEW_MAX);
        fileCount++;
        dirPath = path.join(__dirname, `/data/reviews${fileCount}.txt`);
    }
    if(dishID % 200 === 0 ) {
        // we reached 1 mil
        console.log("write count: ", dishID*REVIEW_MAX);
